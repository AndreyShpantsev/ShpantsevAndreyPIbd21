        ...
        private void FormStorage_Load(object sender, EventArgs e)
        {
            if (id.HasValue)
            {
                try
                {
                    ...
                    var storageDetails = storageList[0].StorageDetails;
                    for (int i = 0; i < storageList.Count; ++i)
                    {
                        if (storageList[i].Id == id)
                        {
                            storageDetails = storageList[i].StorageDetails;
                        }
                    }
                    if (storageDetails != null)
                    {
                        dataGridViewStorage.DataSource = storageDetails;
                        ...
                        dataGridViewStorage.Columns[3].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                    }
                }
                catch (Exception ex)
                {
